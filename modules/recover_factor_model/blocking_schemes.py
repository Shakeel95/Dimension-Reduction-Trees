...
sys.path.append("../")
import treelets.treelet_schemes as trees
import treelets.basis_selection as basis_selection
import recover_factor_model.helper_functions as hf
...
def standard_PCA(panel,n_factors):
    ...
    eig_vectors = hf.ordered_eigen_decomposition(cov)
...
def naive_disjoint_blocks(panel, block_size, n_factors):
    ...
    for i in range(n_blocks):
        ...
        basis[a:b,a:b] = hf.ordered_eigen_decomposition(block_cov)

    energies = hf.variance_score(panel, basis)
...
def naive_sliding_blocks(panel, block_size, n_factors):
    ...
    for i in reversed(range(n+1-block_size)):
        ...
        J = np.eye(n)
        J[a:b,a:b] = hf.ordered_eigen_decomposition(block_cov)
        basis, cov = hf.update_basis_rotate_cov(J, basis, cov)
...
def ranked_disjoint_blocks(panel, block_size, n_factors):
    ...
    for i in range(n_blocks):
        ...
        block = hf.max_weight_clique(cov,k,mask)
        block_cov = hf.build_block_cov(cov,block)

        eig = hf.ordered_eigen_decomposition(block_cov)
        J = hf.build_indexed_Jacobi(block, eig, n)
        basis, cov = hf.update_basis_rotate_cov(J, basis, cov)
...
def ranked_sliding_blocks(panel, block_size, n_factors):
    ...
    for i in range(n+1-block_size):
        block = hf.max_weight_clique(cov,block_size,set())
        block_cov = hf.build_block_cov(cov,block)
...
def from_treelets(panel, n_factors):
    ...
    n, _ = panel.shape
    tree = trees.treelet(panel.T,n,True)
    best_basis = basis_selection.best_basis(tree, n_factors, panel.T)
